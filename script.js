...
function showMeme()
{
    ...
    meme_img.setAttribute('src',memes[getRandomINt(memes.length)]);
    meme_img.setAttribute('alt','meme');
    clearAll();
    memeContainer.appendChild(meme_img);
}

function showJoke()
{
    clearAll();
    //Clearinng the output of the previous action
    const jokeContainer = document.querySelector('.joke-content');
    const joke = document.createElement('p');
    //Creating the paragraph elemnet to show the joke
    joke.textContent=jokes[getRandomINt(jokes.length)];
    //Addng random joke to the joke container
    jokeContainer.appendChild(joke);
    // Adding the joke container to the Webpage
}
...
function showQuote()
{
   ...
   quoteText.textContent=quotes[getRandomINt(quotes.length)].quote;
   quoteAuthor.textContent = quotes[getRandomINt(quotes.length)].author;
...
function showRiddle()
{
    ...
    const randomRiddle = riddles[getRandomINt(riddle.length)];

    const questionContainer = document.createElement("p");

    const answerContainer = document.createElement("p");

    questionContainer.textContent=randomRiddle.question;
    answerContainer.textContent=randomRiddle.answer;
    answerContainer.setAttribute('id',"riddle-answer");
    answerContainer.hidden=true;
    riddleContainer.appendChild(questionContainer);
    riddleContainer.appendChild(answerContainer);

}

function revealAnswers()
{
    const answerContainer = document.querySelector(".riddle-content");
    const riddle = answerContainer.querySelector("p");
    const riddleAnswer = document.querySelector("#riddle-answer");
    
    if(riddle && riddleAnswer.hiddne)
    {
        riddleAnswer.hidden=false;
    }
    else if(riddle && riddleAnswer)
    {
        alert("Answer already exposed");
    }
    else
    {
        alert("There is no riddle to show answer for first click on 'Riddle Me' ");
    }
}
